...
public class Tetromino {
    ...
    public static Color color = Color.rgb(2,60,64);
    ...
    public static void randomColorChanger(int score) {
    	
    	if (score % 10 == 0) {
    		int red = (int)(Math.random() * 125);
    		int green  = (int)(Math.random() * 125);
    		int blue  = (int)(Math.random() * 125);
    ...
    public void rotateBack() {
        pieces.forEach(p -> p.setDirection(p.directions.stream().map(Direction::previous).collect(Collectors.toList()).toArray(new Direction[0])));
    }
